...
DATA_FILE = "D:/Postgraduate/WebSCentinel/dataset/cnn/polling/Google/"
RESULT_FILE = "D:/Postgraduate/WebSCentinel/result/polling/Google/"
obj = "google"
...
for IDX in range(IDX_MIN, IDX_MAX + 1):
    ...
    checkpoint_cb = keras.callbacks.EarlyStopping(patience=20, restore_best_weights=True)
    # clf_cnn.summary()
    clf_cnn.compile(optimizer='nadam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
    clf_cnn.fit(train_X, train_y, epochs=2000, validation_data=(test_X, test_y),verbose=0,callbacks=[checkpoint_cb])

    # acc = clf_cnn.evaluate(test_X, test_y)
    y_pre = clf_cnn.predict(test_X)
...
row_df.to_csv(RESULT_FILE + obj + "_cnn_mean.csv", index=False)
...
row_df.to_csv(RESULT_FILE + obj + "_cnn_std.csv", index=False)