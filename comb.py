...
def getData(DATA_FILE):
    ...
    for pcap_file in os.listdir(DATA_FILE):
        ...
        # local_ip = ips[1]
        # Baidu ip 特殊处理
        # remote_ip = ['110.242.69.21', '110.242.70.57', '220.181.111.1', '39.156.70.239', '39.156.70.46',
        #              '106.117.216.33',
        #              '220.181.111.232', "39.156.66.18", "39.156.66.14", "110.242.68.4", "110.242.68.3", "111.63.60.33",
        #              "220.181.38.149", "220.181.38.150"]
        # # Bing ip 特殊处理
        ...
        #             "192.168.139.129", "192.168.0.105", "192.168.0.101"]  # 本机的ip，就是数据包的目的ip
            ...
            if ip_pkt.haslayer(TCP):
                ...
                if '202.89.233.101' not in remote_ip or '202.89.233.100' not in remote_ip or '13.107.21.200' not in remote_ip\
                    ...
                    if (ip_pkt.src in remote_ip or remote_ip is None) and ip_pkt.dst in local_ip and len(
                            tcp_pkt.payload) != 0 and len(tcp_pkt.payload) <= 2048:
                        pkg_list.append(pkt)
                # 是bing的话单独处理
                else:
...
DATA_FILE = r"D:\Postgraduate\WebSCentinel\data\bing\2013"
CSV_FILE = "../dataset/combinations/Bing/size.csv"
...
for i in range(38, len(col_name)):
    col_name[i] = "chunk_" + str(i - 37)
