...
from polling import extract_from_temporal
...
def getData(DATA_FILE):
    ...
    for pcap_file in os.listdir(DATA_FILE):
        ...
        local_ip = ['192.168.247.139', '192.168.125.100', '192.168.25.129', '192.168.88.128', '192.168.139.131', '192.168.101.129'
                    ...
                    ,"172.30.0.2","172.26.0.2","192.168.0.2","192.168.112.2","192.168.96.2","192.168.32.2","192.168.139.129"]  # 本机的ip，就是数据包的目的ip
        ...
        for pkt in packet_list:
            ...
            if ip_pkt.haslayer(TCP):
                ...
                if '202.89.233.101' not in remote_ip or '202.89.233.100' not in remote_ip or '13.107.21.200' not in remote_ip\
                    ...
                    # print(ip_pkt.src in remote_ip, ip_pkt.dst in local_ip)
        ...
        k = 20
        try:
            _, df_temp = get_temporal_feature(pcap_file)
            poll_features = extract_from_temporal(df_temp, remote_ip, T=T, byte_per_char=byte_per_char, k=k)
            poll_features = poll_features.tolist()
        except Exception as e:
            print(f"[WARN] polling 特征提取失败: {e}")
            poll_features = [0] * 42  # 如果失败，补零
...
DATA_FILE = "../data/bing/2013/"
CSV_FILE = "../dataset/polling_only/bing_2013_comb_polling.csv"
...
print("comb所用时间", (end - start).seconds)
...
# 添加 polling 特征列名
for i in range(38, 38+42):
    col_name[i] = "poll_" + str(i-38)


for i in range(80, len(col_name)):
    col_name[i] = "chunk_" + str(i - 80)
