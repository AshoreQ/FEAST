...
data_names = ["Amazon", "eBay", "Hopkinson Hospital", "JD", "Taobao"]
data = {}
for e in data_names:
    datastr = "E:\\FSCVA实验资料\\FSCVA\\Pcap_processed"  # 修改为你的数据集路径
    # 设置数据集文件夹路径
    dataset_folder = datastr + "\\" + e
    # 初始化载荷大小区间统计字典
    ...
    data[e] = payload_size_intervals['2049+'] / total

df = pd.DataFrame(data, index=["大于2048的占比"])

df.to_excel(r'E:\FSCVA实验资料\FSCVA\统计表格_web程序.xlsx', engine='openpyxl')
